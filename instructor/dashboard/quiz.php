...
<?php 
  ...
  <thead class="text-center bg-primary"style="color:white!important;">
    <tr>
      ...
      <th scope="col">Edit & Manage</th>
    </tr>
  </thead>
  <tbody class="text-center">

<?php 
$Quiz_Object = new backend();
?>
...
$InsData = $Quiz_Object->fetching("instructor","instructor_id",null,"registration_id = $instructor_logId");
if(!empty($InsData)){
    foreach($InsData as $mainDataGet){
    $instructor_main_id = $mainDataGet['instructor_id'];
    // $instructor_main_id = 6;
}
$CourceData = $Quiz_Object->fetching("cource","*",null,"instructor_id = $instructor_main_id");
if(!empty($CourceData)){
    foreach($CourceData as $GetCourceData){
        $courses_required_id = $GetCourceData['courses_id'];        
        $QuizzesData = $Quiz_Object->fetching("quizzes","*",null,"cources_id = $courses_required_id");
        if(!empty($QuizzesData)){
            foreach($QuizzesData as $TotalQuizzes){
                $QA = $TotalQuizzes['Q$A'];
                $quiz_title = $TotalQuizzes['quiz_title'];   
                $cources_id = $TotalQuizzes['cources_id'];
                ...
                $CourceNewData = $Quiz_Object->fetching("cource","*",null,"courses_id = $cources_id");
                if(!empty($CourceNewData)){
                    foreach ($CourceNewData as $Always) {
                        $course_title = $Always['course_title'];
                        $courses_id = $Always['courses_id'];
                ...
                if (!empty($quiz_title)) {
?>
        <td><a href="edit_quiz.php?title=<?=$quiz_title?>&course_id=<?=$courses_id?>">Edit Here</a></td>
<?php                     
                }
    ...
    echo "<td colspan='4'>There is no Quiz yet</td>";
}
}else{
    echo null;
}
